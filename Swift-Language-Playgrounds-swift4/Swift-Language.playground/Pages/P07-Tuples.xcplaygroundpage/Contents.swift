...
var startCoord: (x: Double, y: Double) = (5.1, 8.3)     //1
var endCoord: (x: Double, y: Double) = (375, 667)       //2
var distance = sqrt((endCoord.x - startCoord.x)*(endCoord.x - startCoord.x) + (endCoord.y - startCoord.y)*(endCoord.y - startCoord.y))      //3
print(distance)
var usMoney: (x: Double, y: String) = (1.00, "USD")
var canMoney: (x: Double, y: String) = (1.29, "CAD")
var auMney: (x: Double, y: String) = (1.36, "AUD")

print("\(usMoney.x) \(usMoney.y) is \(canMoney.x) \(canMoney.y)")
print("\(usMoney.x) \(usMoney.y) is \(auMney.x) \(auMney.y)")
